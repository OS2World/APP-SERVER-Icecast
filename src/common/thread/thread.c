...
#if 0 // 2019-05-12
#ifdef __OS2__
/* 2019-02-27 [Digi] Strange bug: thread that calls pthread_rwlock_wrlock()
   will be locked on pthread_rwlock_unlock() if pthread_rwlock_rdlock() called
   from other thread.
   2019-05-12 [Digi] pthread fixed.
 */
    if ( rwlock->sys_rwlock != 0 )
    {
      while( pthread_rwlock_tryrdlock(&rwlock->sys_rwlock) != 0 )
        DosSleep( 1 );
    }
#else
