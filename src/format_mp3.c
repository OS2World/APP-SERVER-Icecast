...
static void mp3_set_tag (format_plugin_t *plugin, const char *tag, const char *in_value, const char *charset)
{
    ...
    if (in_value)
    {
        ...
        // Charset is not NULL when posted via web-interface and NULL when it
        // set by source. In second case we uses configured charset.
        value = util_conv_string (in_value,
                                  charset ? charset : plugin->charset, "utf-8");
#else
...
#ifdef _DIGI
    // We have metadata in utf-8 (see mp3_set_tag()), now we need to convert it
    // in the configured charset.
    if ( source->format->charset != NULL )
    {
      url_artist = util_conv_string( url_artist, "utf-8", source->format->charset );
      url_title = util_conv_string( url_title, "utf-8", source->format->charset );
    }
#endif
...
err:
    if ( source->format->charset != NULL )
    {
      // Free memory that was allocated in util_conv_string().

      if ( url_artist != NULL )
        free( (void *)url_artist );
